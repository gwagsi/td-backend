 ...
 * @author erman
 */
public class SearchTruckFilter {
    
    public static String getQueryForSearchAllAvailableTrucksWithFilter(int sortParam, boolean excludeWeekend, int dotNumber, int ORDER){
        ...
        return "SELECT tt.*, getDistance(tt.`latitude`,tt.`longitude`, ?4, ?5) as distanceWithin FROM ( "
                ...
                + "             AND tt.`truck_axle_ID` >= ?10 "
                + "		AND tt.`general_liability` >= ?11 "
                + "		AND tt.`insurance_liability` >= ?12"
    ...
    public static String getQueryForSearchTruckFilterWithoutDistance(int sortParam, boolean excludeWeekend, int dotNumber, int ORDER){
        ...
        return "SELECT tt.*, tt.`distance` as distanceWithin FROM ( "
                ...
                + "	WHERE  a.`truck_ID` is null"
                + "		" + (dotNumber == 0 ? "" : "AND tt.`d_o_t_number` IS NOT NULL ") 
                + "             AND tt.`truck_axle_ID` >= ?3 "
                + "		AND tt.`general_liability` >= ?4 "
                + "		AND tt.`insurance_liability` >= ?5"
                + "	"
                + "	ORDER BY " + SearchParameter.getSorterParam(sortParam, ORDER)  + " distanceWithin " + (ORDER == 0 ? "ASC" : "DESC") +  ", tt.`creation_date` DESC"
                + ""
                ;
